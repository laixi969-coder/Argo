...
- **第一层 · 每日雷达（广）**：每天自动扫公开数据源，找「真需求 + 有人愿掏钱」的产品机会（含实体 / AI / 虚拟 / 服务），用 `/req` 真需求框架过滤掉伪需求。榜单分为真实需求主榜、待核验证据副榜和 AI 供给副榜；没有可在原帖核验的摘录不得进主榜。
- **第二层 · 按需探讨（深）**：蔡蔡在 Telegram 里直接跟 Argo 对话——「第 3 条深挖」「这条为什么愿付钱」「再找几个类似的」。Argo 带着当天榜单 + 真需求框架的上下文来回答、追问、深挖。

生产日报由 **GitHub Actions** 每天三班运行，写入 Upstash KV；**Vercel 网站**读取 KV 展示内容。Telegram bot 仍可作为推送与对话适配器，但 GitHub Actions 未配 Telegram 凭据时只更新网站，不发送消息。
...
**仍不做（红线）**：未经蔡蔡明确授权的新对外发布、生产部署或外部代发。
...
│   ├── evidence.py     # 校验 LLM 摘录确实出自抓取标题/正文
│   ├── rank.py         # 主榜/待核验/AI 供给三轨排序 → 最多 20 条
...
- 网站 API（`src.web`）是内容和对话枢纽；Telegram 是可选薄客户端。生产网站读取 KV，所有渠道共享同一份机会数据。
- `evidence_quotes` 必须能逐字匹配抓取标题或正文；只有已展示的付款/预算/营收/复购原文可将 `verdict` 标为「市场已验证」。

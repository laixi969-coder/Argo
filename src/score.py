...
6. 操盘手优势契合（决定「真需求」是否=「他的机会」）：本雷达服务的操盘手画像——20 年资深品牌营销与战略、深耕中文市场、擅长把商业逻辑落成多 Agent AI 工具/内容自动化（视频/文案/创意）。
   - 真需求 ≠ 他的机会。需求再真，若要靠重资产/硬件制造/海量地推/纯英文 SaaS 红海拼研发，他没有不公平优势，降权。
   - 命中其优势的加分：品牌·营销·内容创意驱动、中文/华人市场、AI 把创意或专业交付自动化、靠品味与方法论而非纯工程取胜。
   - 这一项影响 score 与 verdict：高契合上调，明显错位即便需求为真也压低评分并在 reason 点明「需求真但非他的赢面」。
...
def _score_one(o, llm):
    try:
        ...
        o["edge_fit"] = str(data.get("edge_fit") or "未知").strip()
    except Exception:
        o["verdict"] = "待验证"
